...
const meetupId = computed(
  () => (route.params.meetupId as string) || "demo-meetup",
);
...
async function startHmsPreview() {
  ...
  try {
        ...
        if (enabled && videoElementRef.value) {
          ...
          setTimeout(async () => {
            try {
              ...
              console.warn(
                "[PreJoinView] Failed to attach video after enabling:",
                error,
              );
            }
          }, 300);
          ...
          if (videoElementRef.value) {
            if (trackID) {
              ...
              try {
                ...
                // eslint-disable-next-line no-console
                console.error(
                  "[PreJoinView] Failed to attach video track:",
                  error,
                );
              }
            } else {
    ...
    if (initialVirtualBackgroundMode === "blur") {
      ...
      initialVirtualBackgroundMode === "image" &&
      initialVirtualBackgroundImageUrl
    ) {
...
watch(videoElementRef, async (newVal) => {
  if (
    ...
    try {
      ...
      // eslint-disable-next-line no-console
      console.warn(
        "[PreJoinView] Failed to attach video on element mount:",
        error,
      );
    }
  }
    ...
    <Card class="w-full max-w-3xl shadow-xl shadow-black/40">
      ...
      <template #content>
        <div v-if="isLoading || isPreviewLoading" class="py-8 text-center text-sm text-slate-300">
          {{
            isLoading
              ? "Preparing your meetup, please wait…"
              : "Loading preview..."
          }}
        </div>
        ...
        <div v-else class="flex flex-col gap-6 lg:flex-row">
          ...
          <div class="flex-1">
            <div class="relative aspect-video overflow-hidden rounded-lg border border-slate-700 bg-black">
              <video ref="videoElementRef" autoplay playsinline muted class="h-full w-full object-cover" />
            ...
            <div class="mt-4 flex flex-col gap-4 text-xs sm:flex-row sm:items-center sm:justify-between">
              <div class="flex items-center gap-3">
                <Button :icon="cameraOn ? 'fa-solid fa-video' : 'fa-solid fa-video-slash'"
                  :severity="cameraOn ? 'success' : 'secondary'"
                  class="flex h-10 w-10 items-center justify-center rounded-full p-0!"
                  :aria-label="cameraOn ? 'Turn camera off' : 'Turn camera on'" @click="handleToggleCamera" />
                <Button :icon="micOn ? 'fa-solid fa-microphone' : 'fa-solid fa-microphone-slash'"
                  :severity="micOn ? 'success' : 'secondary'"
                  class="flex h-10 w-10 items-center justify-center rounded-full p-0!"
                  :aria-label="micOn ? 'Turn microphone off' : 'Turn microphone on'" @click="handleToggleMic" />
              </div>
              <div class="flex flex-1 flex-col gap-1 sm:items-end">
                <div class="flex w-full gap-2 sm:justify-end">
                  <Button label="None" size="small" :severity="store.virtualBackgroundMode === 'none'
                    ? 'success'
                    : 'secondary'
                    " class="flex-1 justify-center sm:flex-none" @click="handleVirtualBackgroundChange('none')" />
                  <Button label="Blur" size="small" :severity="store.virtualBackgroundMode === 'blur'
                    ? 'success'
                    : 'secondary'
                    " class="flex-1 justify-center sm:flex-none" @click="handleVirtualBackgroundChange('blur')" />
                  <Button label="Custom" size="small" :severity="store.virtualBackgroundMode === 'image'
                    ? 'success'
                    : 'secondary'
                    " class="flex-1 justify-center sm:flex-none" @click="handleVirtualBackgroundChange('custom')" />
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="flex w-full flex-col gap-4 lg:w-80">
            ...
            <Button label="Join Meetup" icon="fa-solid fa-right-to-bracket" class="w-full justify-center"
              :disabled="Boolean(localErrorMessage) || isLoading" @click="handleEnterMeetup" />

            <Message v-if="localErrorMessage || store.errorMessage" severity="error" class="mt-1 text-xs">
